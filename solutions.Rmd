...
The robust regression with Student-t error example uses the following prior on the degrees of freedom parameter.[^tprior]
$$
\nu \sim \mathrm{Gamma}(2, 0.01)
$$
...
1. Plot this prior distribution, and the values of the 5th and 95th quantiles. You can use `dgamma(x, 2, scale = 0.01)` and `qgamma(x, 2, scale = 0.01)`. Why does this choice make sense? 
